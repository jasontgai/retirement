    ...
    with st.expander("⚠️ 회원 탈퇴", expanded=False):
        ...
        with st.form("delete_account_form"):
            ...
            with st.container():
                ...
                submitted = st.form_submit_button("회원 탈퇴", use_container_width=True)
...
def show_main_app():
    ...
    with tabs[0]:
        ...
        with col1:
            name = st.text_input("이름", value=st.session_state.get("inp_name", st.session_state.user_name or ""), key="inp_name")
            gender = st.selectbox("성별", ["남", "여"], key="inp_gender")
        with col2:
            birth_year = st.number_input("출생년도", 1940, 2010, 1971, step=1, key="inp_birth_year")
            retirement_age = st.number_input("은퇴희망 연령", 50, 75, 60, step=1, key="inp_retirement_age")

        col3, col4 = st.columns(2)
        with col3:
            birth_month = st.number_input("출생월", 1, 12, 5, step=1, key="inp_birth_month")
        with col4:
            birth_day = st.number_input("출생일", 1, 31, 15, step=1, key="inp_birth_day")

        expected_lifespan = st.slider("기대수명", 75, 100, 90, key="inp_lifespan")
        dependents = st.number_input("부양가족 수", 0, 10, 1, step=1, key="inp_dependents")

        st.divider()
        st.subheader("현재 소득")
        annual_salary = st.number_input("연봉 (만원)", 0, 100000, 8000, step=100, key="inp_salary") * 10000
        annual_bonus = st.number_input("연간 보너스 (만원)", 0, 100000, 1000, step=100, key="inp_bonus") * 10000
        is_employee = st.toggle("직장가입자", key="inp_is_employee")

        st.divider()
        st.subheader("은퇴 후 근로소득")
        st.caption("파트타임·프리랜서 등 은퇴 후 예상 근로소득")
        col_pt1, col_pt2 = st.columns(2)
        with col_pt1:
            parttime_monthly = st.number_input("월 근로소득 (만원)", 0, 10000,
                st.session_state.get("inp_parttime_monthly", 0), step=10, key="inp_parttime_monthly") * 10000
        with col_pt2:
            parttime_until = st.slider("종료 연령", 60, 80,
                st.session_state.get("inp_parttime_until", 70), key="inp_parttime_until")
        ...
        with col_sp1:
            spouse_nps = st.number_input("배우자 국민연금 월수령액 (만원)", 0, 1000,
                st.session_state.get("inp_spouse_nps", 0), step=5, key="inp_spouse_nps") * 10000
            spouse_nps_age = st.slider("배우자 국민연금 개시연령", 60, 70,
                st.session_state.get("inp_spouse_nps_age", 65), key="inp_spouse_nps_age")
        with col_sp2:
            spouse_other = st.number_input("배우자 기타연금 월수령액 (만원)", 0, 1000,
                st.session_state.get("inp_spouse_other", 0), step=5, key="inp_spouse_other") * 10000
            spouse_other_age = st.slider("배우자 기타연금 개시연령", 50, 85,
                st.session_state.get("inp_spouse_other_age", 65), key="inp_spouse_other_age")
...
""")
            ...
            with st.expander("➕ 새 연금 추가", expanded=False):
                ...
                if pn_type_sel in registered:
                    ...
                    if st.button("✏️ 수정하기", key="edit_existing_pn", use_container_width=True):
    ...
    with tabs[3]:
        ...
        living    = st.number_input("기본 생활비 (만원)", 0, 2000, 250, step=10, key="inp_living") * 10000
        medical   = st.number_input("의료비 (만원)", 0, 1000, 50, step=10, key="inp_medical") * 10000
        leisure   = st.number_input("여가/취미 (만원)", 0, 1000, 80, step=10, key="inp_leisure") * 10000
        family    = st.number_input("자녀/부모 지원 (만원)", 0, 1000, 30, step=10, key="inp_family") * 10000
        insurance = st.number_input("보험료 (만원)", 0, 1000, 30, step=10, key="inp_insurance") * 10000
        other     = st.number_input("기타 (만원)", 0, 1000, 20, step=10, key="inp_other") * 10000
    ...
    with tabs[4]:
        ...
        inf_rate = st.slider("물가상승률 (%)", 0.0, 5.0, 2.5, step=0.1, key="inp_inflation") / 100

        if st.button("🔍 은퇴설계 분석하기", type="primary", use_container_width=True):
            ss = st.session_state
            payload = {
                "personal": {
        ...
        if st.session_state.analysis_result:
                ...
                with st.expander("📅 나이별 월수입 시나리오", expanded=True):
                    ...
                    if not _df.empty:
                        ...
                        _area = alt.Chart(_df).mark_area(
                            ...
                            y=alt.Y('월수입(만원):Q', title='월수입 (만원)', stack='zero'),
                        ...
                        st.altair_chart(
                            (_area + _line + _exp_line).properties(height=320),
                            use_container_width=True,
                        )
                        st.caption("🔴 점선: 물가상승 반영 월지출 / 파란선: 월수입 합계")
                    ...
                    for _row in _age_rows:
                        ...
                        if _items != _prev_items or _row == _age_rows[0]:
                            ...
                            st.markdown(
                                f"**{_row['나이']}세~** &nbsp; 월 **{fmt_won(_total)}**{_delta_str}"
            ...
            if nps_scenarios:
                ...
                if 'adj_nps_age' not in st.session_state:
                ...
                if st.button("✅ 이 수급연령 국민연금에 적용", use_container_width=True):
                    for i, p in enumerate(st.session_state.pensions):
                        if p.get('pension_type') == '국민연금':
                ...
                with st.expander("📋 전체 수급시기 시나리오 보기", expanded=False):
                    for s in nps_scenarios:
                        marker = " ⭐" if s['start_age'] == _normal_age else ""
                        sel_marker = " ◀ 선택" if s['start_age'] == sel_nps_age else ""
                        st.text(f"{s['start_age']}세: 월 {fmt_won(s['monthly_amount'])} / 총 {fmt_won(s['total_payout'])}{marker}{sel_marker}")
            ...
            if pension_scenarios:
                ...
                if 'adj_start_age' not in st.session_state:
                ...
                for ps in pension_scenarios:
                    ...
                    if pname not in st.session_state.adj_payout:
                        st.session_state.adj_payout[pname] = cur_y
                ...
                with c2:
                    if adj_surplus >= 0:
                        ...
                        st.warning(f"⬆️ {fmt_won(adj_total_delta)} 개선 — 여전히 {fmt_won(abs(adj_surplus))} 부족")
                    else:
                        st.error("❌ 기간 조정만으로는 부족")

                # 현재 연금에 기간/개시연령 반영 버튼
                if st.button("✅ 조정값 연금에 적용", use_container_width=True):
                    for ps in pension_scenarios:
                        pname = ps['연금명']
            ...
            if _pf_scenarios:
                ...
                for _pf in _pf_scenarios:
                    with st.expander(f"**{_pf['연금명']}** ({_pf['종류']}) — 현재 {_pf['현재_수익률']*100:.1f}%", expanded=True):
                        _cols = st.columns(len(_pf['시나리오']))
                        for _ci, _sc in enumerate(_pf['시나리오']):
                            with _cols[_ci]:
                                _is_cur = _sc['현재설정']
                                _delta = _sc['월수령액_증감']
                                _delta_str = (
                                    f"+{fmt_won(_delta)}" if _delta > 0
                                    else (fmt_won(_delta) if _delta < 0 else "기준")
                                )
                                _label = f"{'✅ ' if _is_cur else ''}{_sc['유형']} ({_sc['수익률']*100:.1f}%)"
                                st.metric(
                                    _label,
                                    fmt_won(_sc['월수령액']),
                                    delta=_delta_str if not _is_cur else None,
                                    delta_color="normal",
                                )
                                st.caption(f"적립금 {fmt_won(_sc['수령시점_적립금'])}")
            ...
            private_scens = result.get('현금흐름_보완', {}).get('사적연금_기간별', [])
            for ps in private_scens:
                pname = ps['연금명']
                kind = ps['종류']
                cur_y = ps['현재_기간']
